...
class TestVWAP:
    ...
    def test_with_start_date(self, quotes_intraday):
        quotes_intraday.sort(key=lambda x: x.date)
        year = 2020
        month = 12
        day = 15
        ...
        results_date = indicators.get_vwap(quotes_intraday[:391], date)
        results_int = indicators.get_vwap(quotes_intraday[:391], year, month, day, hour)
    ...
    def test_bad_data(self, quotes_bad):
        r = indicators.get_vwap(quotes_bad)
        assert 502 == len(r)
    ...
    def test_removed(self, quotes_intraday):
        quotes_intraday.sort(key=lambda x: x.date)

        results = indicators.get_vwap(quotes_intraday[:391]).remove_warmup_periods()
    ...
    def test_condense(self, quotes_intraday):
        quotes_intraday.sort(key=lambda x: x.date)

        results_int = indicators.get_vwap(quotes_intraday[:391], 2020, 12, 15, 10)
