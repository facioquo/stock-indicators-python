...
class TestPivotPoints:
    def test_standard(self, quotes):
        results = indicators.get_pivot_points(
            quotes, PeriodSize.MONTH, PivotPointType.STANDARD
        )
    ...
    def test_camarilla(self, quotes):
        results = indicators.get_pivot_points(
            quotes[:38], PeriodSize.WEEK, PivotPointType.CAMARILLA
        )
    ...
    def test_demark(self, quotes):
        results = indicators.get_pivot_points(
            quotes, PeriodSize.MONTH, PivotPointType.DEMARK
        )
    ...
    def test_fibonacci(self, quotes_intraday):
        results = indicators.get_pivot_points(
            quotes_intraday[:300], PeriodSize.ONE_HOUR, PivotPointType.FIBONACCI
        )
    ...
    def test_woodie(self, quotes_intraday):
        results = indicators.get_pivot_points(
            quotes_intraday, PeriodSize.DAY, PivotPointType.WOODIE
        )
    ...
    def test_bad_data(self, quotes_bad):
        r = indicators.get_pivot_points(quotes_bad, PeriodSize.WEEK)
        assert 502 == len(r)
