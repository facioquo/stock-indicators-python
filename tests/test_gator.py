...
class TestGator:
    def test_standard(self, quotes):
        ...
        assert 481 == len(
            list(filter(lambda x: x.is_upper_expanding is not None, results))
        )
        assert 489 == len(
            list(filter(lambda x: x.is_lower_expanding is not None, results))
        )
    ...
    def test_bad_data(self, quotes_bad):
        r = indicators.get_gator(quotes_bad)
        assert 502 == len(r)
