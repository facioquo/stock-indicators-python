...
class TestChaikinOsc:
    def test_standard(self, quotes):
        ...
        assert 502 - slow_periods + 1 == len(
            list(filter(lambda x: x.oscillator is not None, results))
        )
    ...
    def test_bad_data(self, quotes_bad):
        r = indicators.get_chaikin_osc(quotes_bad, 5, 15)
        assert 502 == len(r)
    ...
    def test_removed(self, quotes):
        ...
        results = indicators.get_chaikin_osc(
            quotes, fast_periods, slow_periods
