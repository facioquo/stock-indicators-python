...
class TestKeltner:
    def test_standard(self, quotes):
        ...
        results = indicators.get_keltner(quotes, ema_periods, multiplier, atr_periods)

        assert 502 == len(results)

        warmup_periods = 502 - max(ema_periods, atr_periods) + 1
        assert warmup_periods == len(
            list(filter(lambda x: x.center_line is not None, results))
        )
        assert warmup_periods == len(
            list(filter(lambda x: x.upper_band is not None, results))
        )
        assert warmup_periods == len(
            list(filter(lambda x: x.lower_band is not None, results))
        )
        assert warmup_periods == len(
            list(filter(lambda x: x.width is not None, results))
        )
    ...
    def test_bad_data(self, quotes_bad):
        r = indicators.get_keltner(quotes_bad, 10, 3, 15)
        assert 502 == len(r)
