...
class TestMAEnvelopes:
    ...
    def test_dema(self, quotes):
        ...
        assert 502 == len(results)
        assert 464 == len(list(filter(lambda x: x.center_line is not None, results)))
        
        r = results[38]
        assert 224.1033 == round(float(r.center_line), 4)
        assert 229.7059 == round(float(r.upper_envelope), 4)
        assert 218.5008 == round(float(r.lower_envelope), 4)
    ...
    def test_tema(self, quotes):
        ...
        assert 502 == len(results)
        assert 445 == len(list(filter(lambda x: x.center_line is not None, results)))
        
        r = results[57]
        assert 222.6022 == round(float(r.center_line), 4)
        assert 228.1673 == round(float(r.upper_envelope), 4)
        assert 217.0372 == round(float(r.lower_envelope), 4)
