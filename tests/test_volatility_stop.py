...
class TestVolatilityStop:
    def test_standard(self, quotes):
        ...
        r = results[54]
        assert 226.2177 == round(float(r.sar), 4)
        assert r.is_stop is False
        assert 226.2177 == round(float(r.upper_band), 4)
        assert r.lower_band is None
        
        r = results[55]
        assert 226.2178 == round(float(r.sar), 4)
        assert r.is_stop is False
        assert 226.2178 == round(float(r.upper_band), 4)
        assert r.lower_band is None
