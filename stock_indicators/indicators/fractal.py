...
class Fractal(Indicator):
    ...
    indicator_method = CsIndicator.GetFractal[Quote]
    chaining_method = None

    list_wrap_class = FractalResults
    unit_wrap_class = FractalResult
...
@calculate_indicator(indicator=Fractal())
def get_fractal(quotes, left_span = None, right_span = EndType.HIGH_LOW, end_type = EndType.HIGH_LOW):
    ...
    if isinstance(right_span, EndType):
        ...
        return (quotes, left_span, right_span.cs_value)
    else:
        return (quotes, left_span, right_span, end_type.cs_value)
