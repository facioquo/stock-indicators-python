...
def get_vwap(quotes: Iterable[Quote], start: Union[datetime, int, None] = None,
             *legacy_date_parts, month: int = 1, day: int = 1,
             hour: int = 0, minute: int = 0) -> "VWAPResults[VWAPResult]":  # pylint: disable=too-many-branches,too-many-statements,keyword-arg-before-vararg
    """Get VWAP calculated.
    ...
    if legacy_date_parts:
        ...
        if len(legacy_date_parts) > 4:
            raise ValueError("Too many positional date arguments (max 4: month, day, hour, minute)")

        # Interpret: start is actually the year; legacy_date_parts supply remaining
        if not isinstance(start, int):
            raise TypeError("Year must be provided as an int when using legacy positional date form")
        year = start
        ...
        if start is not None:
            if isinstance(start, int):
                ...
                raise TypeError(f"start must be int (year), datetime, or None, got {type(start).__name__}")
        else:
            start_dt = None
