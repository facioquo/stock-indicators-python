...
class Chandelier(Indicator):
    ...
    indicator_method = CsIndicator.GetChandelier[Quote]
    chaining_method = None

    list_wrap_class = ChandelierResults
    unit_wrap_class = ChandelierResult


@calculate_indicator(indicator=Chandelier())
def get_chandelier(quotes: Iterable[Quote], lookback_periods: int = 22,
                   multiplier: float = 3, chandelier_type: ChandelierType = ChandelierType.LONG) -> ChandelierResults[ChandelierResult]:
    ...
    """
    return (quotes, lookback_periods, multiplier, chandelier_type.cs_value)